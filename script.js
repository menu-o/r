...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ================================
    // Parallax Effect for RGB Background
    // ================================
    ...
    window.addEventListener('scroll', function() {
        ...
        fans.forEach((fan, index) => {
            ...
            fan.style.transform = `translateY(${scrolled * speed}px) rotate(${scrolled * 0.1}deg)`;
        });
    });
    ...
    // ================================
    // Dynamic Circuit Line Animation
    // ================================
    const circuitLines = document.querySelectorAll('.line');
    
    const animateCircuits = () => {
        circuitLines.forEach((line, index) => {
            setTimeout(() => {
                line.style.background = 'linear-gradient(90deg, transparent, rgba(255, 107, 53, 0.5), transparent)';
                
                setTimeout(() => {
                    line.style.background = 'linear-gradient(90deg, transparent, rgba(255, 107, 53, 0.1), transparent)';
                }, 500);
            }, index * 200);
        });
    };
    
    // Animate circuits every 5 seconds
    setInterval(animateCircuits, 5000);
    ...
    // ================================
    // RGB Fan Speed Control on Scroll
    // ================================
    let lastScrollY = 0;
    let scrollSpeed = 0;
    ...
    window.addEventListener('scroll', () => {
        ...
        // Adjust fan rotation speed based on scroll speed
        const fans = document.querySelectorAll('.fan');
        const speedMultiplier = Math.min(scrollSpeed / 10, 3);
        
        fans.forEach((fan, index) => {
            const baseSpeed = 4 + index;
            const newSpeed = baseSpeed / (1 + speedMultiplier);
            fan.style.animationDuration = `${newSpeed}s`;
        });
    });
